...
def write_grayscale(filename, pixels):
    ...
    Args:
        ...
        pixels: A rectangular image stored as a sequesnce of rows.
    ...
    with open(filename, 'wb') as bmp:
        ...
        #Image Header
        bmp.write(b'\x00\x00\x00\x00')  # Image header size in bytes - 40 decimal
        ...
        for row in reversed(pixels):    # BMP files are bottom to top
            row_data = bytes(row)
            bmp.write(row_data)
            padding = b'\x00' * ((4 - (len(row) % 4)) % 4)    # Pad row to multiple of four _int32_to_bytes
            bmp.write(padding)
...
def dimension(filename):
    ...
    with open(filenamem 'rb') as f:
        magic = f.read(2)
        if magic != b'BM':
            rasie ValueError("{} is not a BMP file".format(filename))
